...
- 77 linked source rows have concise in-site ingredient and step cards; one source remains explicitly unavailable because it is video/social-only or blocked, and six private/household rows have no public source link.
...
- Four Golden household references are intentionally short and flexible: Rice Cooker Rice, Roasted Any Vegetable, Chicken Thighs (Oven or Air Fryer), and Caprese Salad. They are not presented as externally sourced recipes; each has a compact in-site method and is tagged for weekday convenience. Caprese requires fresh tomatoes and is inventory-gated when those are unavailable.
